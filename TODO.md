...
- Make multiple bundle contexts safer for embedded runtimes. Today one package-local bundle can work, but importing a second conflicting bundle in the same process still resolves against the first bundle's shared RubyGems activation state and can crash under `Ruby::Box`.
...
- Revisit whether `import` should always return a `Package::Exports` wrapper with a `default` accessor, or whether the current duck-typed return values are the better long-term fit.
