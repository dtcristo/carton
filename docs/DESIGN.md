...
| File | Role |
| --- | --- |
| `lib/carton.rb` | Entry point, version guard, `Carton.with_bundle` / `Carton.bootstrap_rubygems!`, and internal wiring |
...
| `lib/carton/box.rb` | Box-specific helpers for requiring files, managing export state, and seeding an imported feature's load path when needed |
...
- `Carton.with_bundle` scopes `BUNDLE_GEMFILE` and `BUNDLE_LOCKFILE`, but a parent already running under `bundle exec` is still not a supported activation environment
